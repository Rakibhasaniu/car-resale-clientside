  ...
  const { data: services = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://assegnment-12-server-site.vercel.app/cetegories`
      );
    ...
    <>
      <div className="container my-[100px] ">
        {/* <h1 className="text-4xl text-gray-700 text-bold font-mono py-5 text-center">
          All Categories <span className="text-amber-600">Resell Car</span>{" "}
          Collection{" "}
        </h1> */}
              ...
              >
                <div className="flex justify-between pb-4 border-bottom">
                  <div className="flex items-center">
                    ...
                    >
                      RESELL
                      <span className="text-amber-600"> {item?.company}</span>
                    </a>
                  </div>
                  ...
                  <div className="space-y-2">
                    <a rel="noopener noreferrer" href="#" className="block">
                      <h3 className="text-xl font-semibold dark:text-violet-400">
                        {item?.title.slice(0, 80)} More..
                      </h3>
                    </a>
                    <p className="leading-snug dark:text-gray-400">
                      {item?.paragrap.slice(0, 250)}
                    </p>
                    <div className="flex justify-center mt-3">
                      <Link to={`/cetegories/${item?.id}`} key={item?.id}>
                        <button className="btn md:px-12 btn-primary my-2">
                          viewss all cars
                        </button>
                      </Link>
                    </div>
